...
void all_data_update(void)
{
  // This code is automatically generated for the platform. Please modify each function according to the actual data
    mcu_dp_value_update(DPID_FREQUENCY,frequency_set); //VALUE型数据上报;
    mcu_dp_enum_update(DPID_TASTE,taste_set); //枚举型数据上报;
    mcu_dp_value_update(DPID_VOLTAGE,voltage_get); //VALUE型数据上报;
...
/*****************************************************************************
函数名称 : dp_download_taste_handle
功能描述 : 针对DPID_TASTE的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_taste_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为ENUM
    unsigned char ret;
    unsigned char taste;
    
    taste = mcu_get_dp_download_enum(value,length);
    switch(taste) {
        case 0: taste_set = 1;
        break;
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_enum_update(DPID_TASTE, taste);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
  ...
  ***********************************/
  ...
  switch(dpid)
  {
        ...
        case DPID_TASTE:
            //味道处理函数
            ret = dp_download_taste_handle(value,length);
        break;
...
{
	unsigned short length = 0;
	length = set_bt_uart_buffer(length,(unsigned char code *)MCU_APP_VER_NUM,3);
	length = set_bt_uart_buffer(length,(unsigned char code *)MCU_HARD_VER_NUM,3);
