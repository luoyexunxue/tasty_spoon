...
{
  if (len > 3) return(byte_to_int(value));
	return 0;
}
/*****************************************************************************
...
static uint8_t mcu_common_uart_data_unpack(uint8_t dat)
{
    ...
    case MCU_UART_REV_STATE_FOUND_HEAD:
        bt_uart_rx_buf[UART_RX_Count++] = dat;
    ...
    case MCU_UART_REV_STATE_FOUND_LEN_L:
        bt_uart_rx_buf[UART_RX_Count++] = dat;   //DATA
        uart_data_len--;
        if(uart_data_len==0)
        {
            current_uart_rev_state_type = MCU_UART_REV_STATE_FOUND_DATA;
    ...
    case MCU_UART_REV_STATE_FOUND_DATA:
        bt_uart_rx_buf[UART_RX_Count++] = dat;  //sum data
        ret = TRUE;
        break;
    default:
        my_memset(bt_uart_rx_buf_temp,0,3);
